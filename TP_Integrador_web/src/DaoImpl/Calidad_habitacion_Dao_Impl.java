...
public ArrayList<Calidad_habitacion> ListarDispo(String Fecha1, String Fecha2, String nomb) {
		...
		ArrayList<Calidad_habitacion> x = new ArrayList<Calidad_habitacion>();
			...
			try {
				...
				cn = DriverManager.getConnection(host+dbName, user,pass);
				CallableStatement st = cn.prepareCall("CALL PRO_DisponibilidadPorFechas(?,?,?)");
				...
				while(resultado.next()){
					...
					Calidad_habitacion aux = new Calidad_habitacion();
					aux.setId_calidad_habitacion(resultado.getString("id_habitacion"));
					aux.setNombre(resultado.getString("nombre"));
					aux.setDetalles(resultado.getString("detalles"));
